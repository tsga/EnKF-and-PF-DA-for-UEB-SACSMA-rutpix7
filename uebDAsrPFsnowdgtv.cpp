...
//  State Variables - These serve as initial conditions
	...
	/*if(snowdgtvariteflag == 1)
		...
		std::cout<<statev[ki]<<std::endl;
	std::cout<<Ws<<" "<<Wc<<std::endl;
	*/
    if(Us <= 0.0)
	{
		refDepth = 0.0;
...
//DGT Daily average temperatures handled internally so that multiple time steps will work
	...
	//  if(snowdgtvariteflag == 1)
	  //std::cout<<Tssk_ave<<" "<<Tsavek_ave<<std::endl;
...
//   ifany of these variables are out of range due to any problem set them back to freezing
	  ...
	  {
	    if(snowdgtvariteflag == 1)		
	        std::cout<<"Invalid previous time step surface temperature set to 273 K"<<Tssk_old<<std::endl;
		Tssk_old = T_k;  
	  }
	  if(Tsavek_old  < 0)
	  {
	    if(snowdgtvariteflag == 1)		
	        std::cout<<"Invalid previous time step average temperature  set to 273 K"<<Tsavek_old<<std::endl;
		Tsavek_old  = T_k;  
	  }
	  if(Tssk_ave  < 0)
	  {
	    if(snowdgtvariteflag == 1)		
	        std::cout<<"Invalid last 24 hr average surface temperature  set to 273 K"<<Tssk_ave<<std::endl;
		Tssk_ave  = T_k;  
	  }
	  if(Tsavek_ave  < 0)
	  {
	    if(snowdgtvariteflag == 1)		
	        std::cout<<"Invalid last 24 hr average temperature set to 273 K"<<Tsavek_ave<<std::endl;
		Tsavek_ave  = T_k;  
	  }	 
...
//  Calculate Albedo	
		...
		if (iflag[3] == 1)
		{
			...
			//8.7.18 modis albedo modifier---half used (for vis Del)   0.01 to convert % to decimal
			Alb = (1.0 - 0.5*0.01*modisAlbedoFact) * ALBEDO(statev[2], cosZen, (Ws - WGT) / RRHO, Aep, abg, avo, anir0);      // Snow depth (Ws/RRho)
...
//            ENDIF
		...
		{
			Alb = statev[2];			//--assume alb observed accounts for 'all' effects: Alb = (1.0 - 0.01*0.5*modisAlbedoFact) * statev[2];
		}
...
//  However this model is not suitable to model the rainfall interception and throughfall
	...
	if(Cc > 0)
	  ...
	  Inmax = 0;	
	//if(snowdgtvariteflag == 1)
	  //std::cout<<LAI<<" "<<Cc<<" "<<" "<<Sbar<<" "<<Rhofs<<" "<<S<<" "<<Inmax<<std::endl;
...
//10.13.16 	//if SWE <= 0: SWE, tausn, dlSage, refD and trefD must be 0 
	//std::cout << statev[2] << std::endl;
	if (Ws <= 0)
	{
		Ws = 0.0;
		if (iflag[3] == 1)
	...
	if (snowdgtvariteflag == 1)
		std::cout <<"Tausn = "<< statev[2] << std::endl;
...
//yjs update tsbackup and tavebackup
	...
	//printf("Us = %f   Ws =   %Ws", Us, Ws);
      ...
      OutArr[7] = QEs;
      OutArr[8] = Es * 1000.0;
      OutArr[9] = SWIT * 1000.0;
      OutArr[10] = QMs;
      OutArr[11] = Q;
      OutArr[12] = FM;
      OutArr[13] = Tave;
      OutArr[14] = TSURFs;
      OutArr[15] = cumP * 1000.0;
      OutArr[16] = cumEs * 1000.0;
      OutArr[17] = cumMr * 1000.0;
      OutArr[18] = Qnet;
      OutArr[19] = smelt * 1000.0;
      OutArr[20] = refDepth * 1000.0;
      OutArr[21] = totalRefDepth * 1000.0;
      OutArr[22] = cf;
      OutArr[23] = Taufb;
      OutArr[24] = Taufd;
      OutArr[25]=Qsib;
      ...
      OutArr[48] = Mc * 1000.0;
      OutArr[49]=FMc;
      OutArr[50] = SWIGM * 1000.0;
      OutArr[51] = SWISM * 1000.0;
      OutArr[52] = SWIR * 1000.0;       
  //2    continue
       statev[0]=Us;
       statev[1]=Ws;
	   statev[4]=refDepth;
...
//************************  Predictor CORRECTOR ***********************************
 __host__ __device__  
void  uebCellDA::PREDICORRc ( float &Us, float &Ws, float &Wc, float Alb, float dt, float rid, float P, float Pr, float Ps, float Ta, float V, float RH, float Qsi, float Qli,
...
{
//##=======================1.31.18 for when there is evaporation yet there is no snow#
	float Ws_prev = Ws;
	float Wc_prev = Wc;                                              //#     
//##=================================================================================#
	float    int1, Mc1, Mr1;	
...
//   Save values so that they can be averaged for output
	...
	{
		std::cout<<"\n Predictor: Us1, Ws1, Ts1, Q1, FM1 "<<std::endl;
		std::cout<<"   "<< Us1<<" "<< Ws1<<" "<<TSURFs1<<" "<<Q1<<" "<<FM1<<std::endl<<std::endl;
	}
...
//     CORRECTOR FOR SUB_CANOPY SNOW
	...
	if(snowdgtvariteflag2 == 1)
	{
		std::cout<<"\n Corrector: Us2, Ws2, Ts, Q, FM "<<std::endl;
		std::cout<<"   "<< Us2<<" "<< Ws2<<" "<<TSURFs<<" "<<Q<<" "<<FM<<std::endl<<std::endl;
	}
...
//    Check that nothing went wrong
                if(MR < 0)
				{
	               std::cout<<"Error - negative melt rate in snow"<<std::endl;
				   getchar();
				}
	            if(Ws2 <  0)
				{
	               std::cout<<"Error - negative w2 in snow"<<std::endl;	
				   getchar();
				}
			    Q = Ae/dt-QMs;
//   Now set first pass values equal to final values to fake the averages below  #$%^****take?
		 ...
		 {
			//if( niter == 1)
			std::cout<<std::endl<<"Iteration in pred-cor: niter, Us2, Ws2, Ts, Q, FM: "<<std::endl;
			std::cout<<"  "<<(niter-1)<<" "<<Us2<<" "<<Ws2<<" "<<TSURFs<<" "<<Q<<" "<< FM<<std::endl;
		 }
...
//##1.31.18 ====================for when there is evaporation yet there is no snow:##
	float dW_t = (Ws + Wc - Ws_prev - Wc_prev) / dt;                               //#
	if (Ws_prev <= 0 && Es > 0)                                                    //#
	{
		Es = 0.0;
		MR = P - Es - Ec - dW_t;
		if (MR < 0)
		{
			if (snowdgtvariteflag == 1)
			{
				std::cout << "Error! -ve Surface melt " << MR << " at grid cell: " << uebCellDAY << " " << uebCellDAX << std::endl;
				std::cout << "datetime: " << mtime[0] << "-" << mtime[1] << "-" << mtime[2] << ":" << mtime[3] << std::endl;
			}
			MR = 0;
		}
	}	                                                                           //#
...
/*
//##1.31.18 ====================for when there is evaporation yet there is no snow:##
	float dWs_t = (Ws - Ws_prev) / dt;                               // dW/dt at surface
	float dWc_t = (Wc - Wc_prev) / dt;                               // dW/dt at canopy
	if (Wc <= 0 && Ec > 0)  // no canopy snow but there is Ec
	{
		Wc = 0.0;
		Ec = 0.0;		
		// for mass balance
		MR = P - Es - Ec - dWc_t - dWs_t;
		if (MR < 0)
		{
			//if (snowdgtvariteflag == 1)
			{
				std::cout << "Error! -ve Canopy melt "<< MR <<" at grid cell: " << uebCellDAY << " " << uebCellDAX << std::endl;
				std::cout << "datetime: " << mtime[0] << "-" << mtime[1] << "-" << mtime[2] << " " << mtime[3] << std::endl;
			}
			MR = 0;
		}
	}
	if (Ws <= 0 && Es > 0)  //no surf snow but there is Es
	{
		//Ws = 0.0;
		if (Ws_prev <= 0)    //both Ws_prev and Ws zero
		{
			Es = 0.0;
		}
		else if (Es > (-1.0*dWs_t))  // evap is > W avail
			Es = -1.0*dWs_t;       //dWs_t is negative as Ws = 0 and Ws_prev is non zero
...
//				CALCULATE CANOPY MASS FLUX AT ANY INSTANT
 __host__ __device__  
void  uebCellDA::QFMc(float Us, float Ws, float Wc, float A, float dt, float rid, float P, float Pr, float Ps, float Ta, float V, float RH, float Qsi, float atff, float Qli,
...
__host__ __device__
float uebCellDA::SRFTMPSC(float &Tssk, float Us, float Ws, float Wc, float A, float dt, float P, float Pr, float Ps, float Ta, float V, float RH, float Qsi, float atff, float cf, float Qli, float cosZen,
...
/*15*/      while (niter < nitermax)
		...
		//      Jacobian matrix
				...
				if(snowdgtvariteflag == 1)
				{
					if (niter == 1)
					{
						std::cout<<"Iteration solving non-linear eqn. in 2D"<<std::endl;
						std::cout<<"F1, F2, F1ts,  F2ts, F1tc, F2tc"<<std::endl;      //, J11, J12, J21, J22 "<<std::endl;
						std::cout<<"S1, S2, Tssk1, Tck1"<<std::endl;					
						//std::cout<<"S1num, S2num, S1denom, S2denom"<<std::endl;
					}
					std::cout<<F1<<" "<<F2<<" "<<F1ts<<" "<<F2ts<<" "<<F1tc<<" "<<F2tc<<std::endl; //		std::cout " "<<J11<<" "<<J12<<" "<<J21<<" "<<J22<<std::endl;
					std::cout<< S1<<" "<< S2<<" "<< Tssk1<<" "<< Tck1<<std::endl;
					//std::cout<<S1num<<" "<< S2num<<" "<< S1denom<<" "<< S2denom<<std::endl;
				}
...
/*18*/  ER    = tol_l*2;                           // so that it does not end on first loop // This is the start when LAI is zero
		...
		{  
			...
			if(F1==F2)              //9.17.13
			{
				...
				{					
					//F1 += (0.01*F2); 
					std::cout<<"  Warning! F1 and F2 have the same value, can lead to v. large surface temp Tssk"<<std::endl;					
					std::cout<<"F1, F2, Tssk, Tslast "<<std::endl;
					std::cout<<"  "<<F1<<" "<<F2<<" "<<Tssk<<" "<<Tslast<<std::endl;
					getchar();
			    }				
			...
			{
				if (niter == 0)
						std::cout<<"Surface temp iteration for soln in 1D: F1, F2, Tssk, Tslast "<<std::endl;
				std::cout<<"  "<<F1<<" "<<F2<<" "<<Tssk<<" "<<Tslast<<std::endl;
			}
			if(Tssk < Tak - 50) 
		...
		{                                    //   ifstill not converged use bisection method
			...
			if(Flb*Fub > 0)     // these are of the same sign so the range needs to be enlarged // an almost ridiculously large range - solution should be in this ifit exists
			{
				...
				if( Flb*Fub > 0)   // these are of the same sign so no bisection solution
				{
					//if(snowdgtvariteflag == 1)
					{
						std::cout<<"Bisection surface temperature solution failed with large range"<<std::endl;
						std::cout<<"datetime, model element:"<< mtime[0]<<mtime[1]<<mtime[2]<<mtime[3]<<"  "<<uebCellDAY<<" "<<uebCellDAX<<std::endl;   //mtime[4]<<std::endl;					
						std::cout<<" A canopy temperature of 273 K assumed"<<std::endl;
					}
					Tssk=Tk;
					goto labl10;
				} 
				else
				{
					if(snowdgtvariteflag == 1)
					{
						std::cout<<"Bisection surface temperature solution with large range"<<std::endl;
						std::cout<<"datetime, model element:"<< mtime[0]<<mtime[1]<<mtime[2]<<mtime[3]<<"  "<<uebCellDAY<<" "<<uebCellDAX<<std::endl;   //mtime[4]<<std::endl;					
						std::cout<<" This is not a critical problem unless it happens frequently"<<std::endl;
						std::cout<<" and solution below appears incorrect"<<std::endl;
					}
				}	
			}                        //else      endif
			//     Here do the bisection
			niter = logf((Tub-Tlb)/tol_l)/logf(2.0);   // Number of iterations needed for temperature tolerance
			//if(snowdgtvariteflag == 1) 	std::cout<<"Bisection iter: "<<niter<<std::endl;
			...
			if(ibtowrite == 1)
				if(snowdgtvariteflag == 1)
				{
					std::cout<<"Surface temperature: "<<Tssk<<std::endl;
				    std::cout<<"Energy closure: "<<F1<<std::endl;
...
labl10:
		...
		iterC++;
		if(snowdgtvariteflag == 1)
			std::cout<<" iterC: "<<iterC<<" ERc: "<<ERc<<std::endl;
...
__host__ __device__ 
float uebCellDA::CanTemp(float& Tck, float Us, float Ws, float Wc, float A, float dt, float P, float Pr, float Ps, float Ta, float V, float RH, float Qsi, float atff, float cf, float Qli,
...
////       Tck  = TaK                              // first approximation
       ...
       while((ER  > tol_l) && (niter < nitermax))
       {
		  ...
		  if(F1==F2)
		  {
			  ...
			  {
				  f1minf2 = F1 - F2;
				  std::cout<<"  Warning! F1 and F2 have the same value, can lead to v. large canopy temp Tck"<<std::endl;
				  std::cout<<" F1 - F2: " <<f1minf2<<std::endl;
			  }
			  goto labl11;                                     //10.30.13 go to bisection if F1 == F2 as div by zero results in #IND for Tssk	
		  }
		  ...
		  {   
			  if (niter == 0)
						std::cout<<"Canopy temp iteration for soln in 1D: F1, F2, Tck, Tclast "<<std::endl;
			  std::cout<<"  "<<F1<<" "<<F2<<" "<<Tck<<" "<<Tclast<<std::endl;
		  }
...
labl11:
	     if(ER > tol_l)                               // If the solution has converged, skip the following
		 {
			  ...
			  if(Flb*Fub > 0)       // these are of the same sign so the range needs to be enlarged // an almost ridiculously large range - solution should be in this ifit exists
			  {
				  ...
				  if(Flb*Fub > 0)   // these are of the same sign so no bisection solution
				  {
					  //if(snowdgtvariteflag == 1)
					  {
						  std::cout<<"Bisection canopy temperature solution failed with large range"<<std::endl;
						  std::cout<<"datetime, model element:"<< mtime[0]<<mtime[1]<<mtime[2]<<mtime[3]<<"  "<<uebCellDAY<<" "<<uebCellDAX<<std::endl;   //mtime[4]<<std::endl;				
						  std::cout<<" A canopy temperature of 273 K assumed"<<std::endl;
					  }
					  Tck=Tk;
				  ...
				  else
				  {
					   if(snowdgtvariteflag == 1)
					  {
						  std::cout<<"Bisection canopy temperature solution with large range"<<std::endl;
						  std::cout<<"datetime, model element:"<< mtime[0]<<mtime[1]<<mtime[2]<<mtime[3]<<"  "<<uebCellDAY<<" "<<uebCellDAX<<std::endl;   //mtime[4]<<std::endl;			
						  std::cout<<" This is not a critical problem unless it happens frequently"<<std::endl;
						  std::cout<<" and solution below appears incorrect"<<std::endl;
					  }
	...
	//     Here do the bisection
		   niter = logf((Tub-Tlb)/tol)/logf(2.0);   // Number of iterations needed for temperature tolerance
		   for (int iter =0; iter< niter; ++iter)
		   {
			   Tck = 0.5*(Tub + Tlb);
			   F1   =  SURFEBC (Tck,Us,Ws,Wc,A,dt,P,Pr,Ps,Ta,V,RH,Fs,cf,Qli, Qsi,atff,cosZen,EmC,Ems,param,sitev,iradfl,Qnetob,iTsMethod,mtime,
		   ...
		   if(ibtowrite == 1)
			  if(snowdgtvariteflag == 1)
			  {
				  std::cout<<"Canopy temperature: "<<Tck<<std::endl;
				  std::cout<<"Energy closure: "<<F1<<std::endl;
				  std::cout<<"Iterations:"<<niter<<std::endl;
			  }		   
...
__host__ __device__ 
float uebCellDA::SURFEBSC(float Tssk, float Us, float Ws, float Wc, float A, float dt, float P, float Pr, float Ps, float Ta, float V, float RH, float Fs, float Cf, float Qli, float Qsi,
...
//     Parameters
	float cg = param[3],				    //  Ground heat capacity [nominally 2.09 KJ/kg/C],
	 ...
	 if(snowdgtvariteflag2 == 1)
	  {
		  std::cout<<"Inputs to surfebsc"<<std::endl;
		  std::cout<<std::setprecision(15)<<Tssk<<" "<<Us<<" "<< Ws<<" "<< Wc<<" "<< A<<" "<< dt<<" "<< P<<" "<< Pr<<" "<< Ps<<" "<< Ta<<" "<< V<<" "<< RH<<" "<< Fs<<" "<< Cf<<" "<< Qli<<" "<< Qsi<<" ";
          std::cout<<std::setprecision(15)<<atff<<" "<< cosZen<<" "<< EmC<<" "<< Ems<<" "<< iradfl<<" "<< Qnetob<<" "<<iTsMethod<<std::endl;
	  }
//     07/25/02   at Boise.  To make the UEB2 work under Unix/linux system the fancy stuff like "Select case" shall not be used
	...
	if(snowdgtvariteflag2 == 1)
		std::cout<<std::setprecision(15)<<"qcs,Ess,Esc:"<<qcs<<" "<<Ess<<" "<<Esc<<std::endl;
	...
	if(iradfl == 0) 
	{
	  ...
	  if(snowdgtvariteflag2 == 1)
	  {
		 std::cout<<"Outputs from surfebsc"<<std::endl;
		 std::cout<<std::setprecision(15)<<Qpc<<" "<< Qps<<" "<< Inmax<<" "<<Rkinc<<" "<< Rkinsc<<" "<< Vz<<" "<< Tck<<" "<< Tk<<" ";
	     std::cout<<std::setprecision(15)<<Tak<<" "<<EA<<" "<<RHOA<<" "<< fkappaS<<" "<< RHO<<" "<< TherC<<" "<< TSURFs<<" ";
		 std::cout<<std::setprecision(15)<<Tave<<" "<<refDepth<<" "<<Qp<<" "<<QHs<<" "<<QEs<<" "<< qcs<<" "<<Qnetob<<" "<<Qsns<<" "<<Qlns<<" "<<Qsnc<<" "<<Qlnc<<" "<<Qlis<<" "<<SURFEBSC_v<<std::endl;
	  }
	return SURFEBSC_v;
}
...
__host__ __device__ 
float uebCellDA::SURFEBC(float Tck, float Us, float Ws, float Wc, float A, float dt, float P, float Pr, float Ps, float Ta, float V, float RH, float Fs, float Cf, float Qli, float Qsi,
                  float atff,float cosZen,float EmC,float Ems,float *param,float *sitev,int iradfl,float Qnetob,int iTsMethod,float *mtime,float QPc,float &QPs,float &Inmax,
...
__host__ __device__ 
float uebCellDA::QcEst(float Ws, float P, float Tssk, float &Tck, float V, float &Zm, float &d, float &Z0c, float &Rimax, float &Rcastar, float Cf, float Fs, float Qli,
	...
	float Taufb, Taufd, Qsib, Qsid,
		    Betab, Betad, Taub, Taud,
			  Qsns, Qsnc,
			    Qlis,Qlns,Qlnc;
	float Rkinc, Tac;
//some of the variables in the fortran version  declared here but never used are deleted (e.g. LAI, cc,...)
...
__host__ __device__ 
float uebCellDA::FMELT(float UB, float Rho_w, float W, float HF, float LC, float RID, float KS, float PRAIN)
{
      float  FMELT_v, UU, SS;
//       write(*,*) 'I am in FMELT////////'
      UU = 0.0;
	  ...
	  else
	  {
         ...
         FMELT_v = KS*powf(SS,3.0);
	  }

      if(FMELT_v < 0)
	  {
          std::cout<<"FMELT is NEGATIVE"<<std::endl;
	      getchar();
	  }    
      return FMELT_v;
}
...
// Linear simple function to calculate the gradient of Q and T
 __host__ __device__  
void    uebCellDA::Grad(float qc1, float qc2,float t1,float t2, float &a,float &b)
{
	if((t2-t1) != 0) 
	{
	  b = (qc2-qc1)/(t1-t2);
...
//     function to calculate the refreezing depth
 __host__ __device__ 
float  uebCellDA::refDep(float flans, float a, float b, float hf, float  rhom, float dt, float x1)
{
	float temp, refDep_v;
...
//******************** CALCULATE THE AVE.TEMP.OF SNOW AND INTERACTING SOIL LAYER ********************
 __host__ __device__ 
float uebCellDA::TAVG(float UB, float W, float RHOW, float CS, float TO, float RHOG, float DE, float CG, float HF)
{
...
//yjs  Calculate the daily average value //yjs  n number of records, a minimum value -100 or some
 __host__ __device__ 
float uebCellDA::daily_ave(float *backup, int n, float a)
{
...
//	Function to get the LanE which is the thermal conductivity by ze
 __host__ __device__ 
float  uebCellDA::LanE(float LanS, float LanG, float Zs, float rho, float rhog, float cs, float cg, float r, float &ze, float w1day)
{
...
//     Function to calculate Albedo.   BATS Albedo Model (Dickinson et. al P.21)
 __host__ __device__ 
float uebCellDA::ALBEDO(float tausn, float cosZen, float d, float aep, float abg, float avo, float airo)     // snow age, zenith angle, snow depth
...
//     Function to calculate Dimensionless age of snow for use in BATS Albedo Model (Dickinson et. al P.21)
 __host__ __device__  
void    uebCellDA::AGESN(float &tausn, float dt, float Ps, float tsurf, float tk, float dNewS) 
{	     
...
//     Partitioning of precipitation into rain and snow      
 __host__ __device__ 
float uebCellDA::PARTSNOW(float P, float TA, float TR, float TS)
{
	float PARTSNOW_v;
    if(TA < TS)
        PARTSNOW_v = P;
