...
__host__ __device__  
void uebCellDA::UPDATEtime(int &YEAR, int &MONTH, int &DAY, double &HOUR, double DT)
{
...
__host__ __device__   
void uebCellDA::atf(float &atff,float trange,int month, float *dtbar, float a, float c)
{
	//DIMENSION dtbar(12)
...
__host__ __device__   
void uebCellDA::hyri(int YEAR, int MONTH, int DAY, float HOUR, float DT, float SLOPE, float AZI, float LAT, float &HRI, float &COSZEN)
{
	float  LP,LAT1;   
...
__host__ __device__  
int uebCellDA::julian(int yy, int mm, int dd)
{
	int julian;
...
__host__ __device__   
void uebCellDA::cloud(float as, float bs, float atff, float &cf)
{
...
//TBC_6.5.13
__host__ __device__   
void  uebCellDA::qlif(float TA, float RH, float TK, float SBC, float &Ema, float &Eacl, float cf, float &qliff )
{
	float TAK  = TA + TK;
	float EA   = RH * svpw(TA);
	//******************************************************  old option
...
__host__ __device__ 
double uebCellDA::julian(int I, int M, int K, double H)
{
	double TJD,JD;
...
__host__ __device__   
void  uebCellDA::calendardate (double TJD,int &I,int &M,int &K, double &H)
{
	double DJD, JD;
	int L, N;
	DJD = TJD + 0.5; 
